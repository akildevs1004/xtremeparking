    ...
    <v-container class="mx-auto py-4" style="max-width: 480px">
      ...
      <v-card class="rounded-xl pa-3 mb-3 card-dark">
        <div class="caption text--secondary mb-1">
          Enter Vehicle Number (Ex: <strong>B123456</strong>)
        </div>
        <v-text-field
      ...
      <v-slide-y-transition v-if="details">
        <div v-if="details">
          ...
          <v-card class="rounded-xl pa-4 mb-3 card-dark text-center">
            ...
            <div class="veh-num mt-1">
              {{ details.vehicle.log_vehicle_number }}
            </div>
          </v-card>
          <v-row>
            <v-col cols="6">
              ...
              <v-card class="rounded-xl pa-3 mb-3 card-dark text-center">
                ...
                <div class="parkingTime" style="font-size: 15px">
                  {{
                    details.vehicle.in_time
          ...
          <v-card class="rounded-xl pa-3 mb-3 card-dark text-center">
            <div class="parkingTime">
              {{ humanDuration(details.fee.duration_in_minutes) }}
            </div>
          ...
          <!-- Total Price -->
          <v-card
            class="rounded-xl pa-3 mb-3 card-dark text-center"
            style="border: 1px solid green; font-size: 25px"
          >
            <div class="parkingTime">
              {{ currency }} {{ price(details.fee.total_amount) }}
            </div>
            <div class="parkingLabel mt-1">
              Total Price ({{ details.fee.duration_in_hours }} X
              {{ details.fee.parking_price_per_hour }} rate)
            </div>
          </v-card>
          <!-- Total Price -->
          <v-card
            v-if="!paymentPopupDisplay"
            class="rounded-xl pa-3 mb-3 card-dark text-center"
            style="border: 1px solid green; font-size: 25px"
          >
            <Creditcard
              @success="successEvent"
              v-if="details?.fee.total_amount > 0 && !cardPaymentStatus"
              :displayAmount="details.fee.total_amount"
              :parkingid="details.vehicle.id"
            />

            <div v-else style="font-size: 16px; color: green">
          ...
          <v-card
            v-if="details?.fee.total_amount > 0 && paymentPopupDisplay"
            class="rounded-xl pa-3 mb-3 card-dark text-center"
            style="border: 1px solid green; font-size: 25px"
          >
            <div
              v-if="!dialogParkingReceipt && !details.vehicle.payment_datetime"
            >
              ...
              <v-btn
                @click="startPopupPayment(details.fee.total_amount)"
                block
                x-large
                v-if="!dialogParkingReceipt"
                class="rounded-xl gate-btn mt-3 d-flex align-center justify-center"
                color="success"
              >
                <span>
              ...
              <div v-if="!popupPaymentSuccessreceipt && popupTriggered">
                <div class="pt-2" style="font-size: 15px; color: red">
                  {{ popupPaymentMessage }}
                </div>
              </div>
            </div>
            <div class="pt-2" v-else style="font-size: 16px; color: green">
              Payment Received. Thank you
            </div>
          </v-card>
        </div>
    ...
    async fetchDetails() {
      ...
      try {
        ...
        if (data.action == "parking_qr_get_vehicle_details") {
          ...
          if (data.data.vehicle.payment_datetime || !data.data.fee) {
            // vehicle out is captured already - No In found
            this.keyId++;
            ...
            if (data.paid && data.paidAt)
              this.startBufferCountdown(data.paidAt);
          }
        }
      } catch (e) {
      ...
      if (payment_sessionid) {
        ...
        if (data.action == "/stripe/session") {
          ...
          const data2 = await mqttRequestReply({
            ...
            payload: {
              fee: this.details.fee,
              payment_response: data.payment_intent,
              parking_id: this.parkingLogid,
            },
            timeoutMs: 8000,
          });
...
.veh-num {
  ...
  font-size: 32px;
  color: #5bd3ff;
  text-shadow: 0 0 10px rgba(91, 211, 255, 0.55),
    0 0 20px rgba(91, 211, 255, 0.35);
}
