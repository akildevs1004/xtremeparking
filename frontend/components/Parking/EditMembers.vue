          ...
          <v-row>
            <v-col cols="4">
              <div class="text-center mt-0">
                ...
                <v-btn class="mt-2  " color="primary" style="width: 50%" small @click="onpick_primary_attachment">{{
                  !editId ?
                    "Upload" :
                    "Change" }}
            ...
            <v-col cols="8">
              <v-row class="pt-0">
                <v-col cols="6" dense>
                  <v-text-field label="First Name" dense small outlined type="text" v-model="payload_members.first_name"
                ...
                <v-col cols="6" dense>
                  <v-text-field label="Last Name" dense small outlined type="text" v-model="payload_members.last_name"
                    hide-details :readonly="!editable" :filled="!editable"></v-text-field>
                ...
                <v-col cols="6" dense>
                  <v-text-field label="Contact Number" dense small outlined type="number"
                    v-model="payload_members.phone" hide-details :readonly="!editable"
                    :filled="!editable"></v-text-field>
                  <span v-if="primary_errors && primary_errors.phone" class="text-danger mt-2">{{
                    primary_errors.phone[0] }}</span>
                </v-col>
                <v-col cols="6" dense>
                  <v-text-field label="Email ID(Login name)" dense small outlined clearable autocomplete="off"
                    v-model="payload_members.email" hide-details :readonly="!editable"
                    :filled="!editable"></v-text-field>
                ...
                <v-col cols="6" dense>
                  ...
                  <v-select :disabled="!editable" label="Member Type" v-model="payload_members.member_type"
                    @change="onMemberTypeChange()" :items="[
                      {
                    ...
                    ]" dense placeholder="Membsership" outlined :hide-details="true" item-text="description"
                    item-value="name"></v-select>

                  <span v-if="primary_errors && primary_errors.member_type" class="text-danger mt-2">{{
                    primary_errors.member_type[0] }}</span>
                </v-col>
                <v-col cols="6" dense>
                  <v-text-field :filled="!editable" label="Address" dense small outlined clearable autocomplete="off"
                    v-model="payload_members.address" hide-details :readonly="!editable"></v-text-field>
                  <span v-if="primary_errors && primary_errors.address" class="text-danger mt-2">{{
                ...
                <v-col cols="6" v-if="member_type == 'Tenant'">
                  <v-menu v-model="from_menu" :close-on-content-click="false" transition="scale-transition" offset-y
                    max-width="290px" min-width="auto">
                    <template v-slot:activator="{ on, attrs }">
                      <v-text-field label="Start Date" hide-details v-model="payload_members.membership_start"
                        persistent-hint append-icon="mdi-calendar" readonly outlined dense v-bind="attrs" v-on="on"
                        :disabled="!editable"></v-text-field>

                    </template>
                    <v-date-picker style="min-height: 320px" v-model="payload_members.membership_start" no-title
                ...
                <!-- TO -->
                <v-col cols="6" v-if="member_type == 'Tenant'">
                  <v-menu v-model="to_menu" :close-on-content-click="false" transition="scale-transition" offset-y
                    max-width="290px" min-width="auto">
                    <template v-slot:activator="{ on, attrs }">
                      <v-text-field label="End Date" hide-details v-model="payload_members.membership_end"
                        persistent-hint append-icon="mdi-calendar" readonly outlined dense v-bind="attrs" v-on="on"
                        :disabled="!editable"></v-text-field>

                    </template>
                    <v-date-picker style="min-height: 320px" v-model="payload_members.membership_end" no-title
                      @input="to_menu = false"></v-date-picker>
                  </v-menu>
                </v-col>
                <v-col cols="6" dense>
                  <v-text-field :filled="!editable" label="Plate Number" dense small outlined clearable
                    autocomplete="off" v-model="payload_members.plate_number" hide-details
                    :readonly="!editable"></v-text-field>
                  <span v-if="primary_errors && primary_errors.plate_number" class="text-danger mt-2">{{
                    primary_errors.plate_number[0] }}</span>
                </v-col>

                <v-col cols="6" dense>
                  <v-text-field :filled="!editable" label="Vehicle Country Region/Country" dense small outlined
                    clearable autocomplete="off" v-model="payload_members.vehicle_country_region" hide-details
                    :readonly="!editable"></v-text-field>
                  <span v-if="primary_errors && primary_errors.vehicle_country_region" class="text-danger mt-2">{{
                    primary_errors.vehicle_country_region[0] }}</span>
                </v-col>

                <v-col cols="6" dense>
                  <v-text-field :filled="!editable" label="Plate Type(Private/Public etc.)" dense small outlined
                    clearable autocomplete="off" v-model="payload_members.vehicle_plate_type" hide-details
                    :readonly="!editable"></v-text-field>
                  <span v-if="primary_errors && primary_errors.vehicle_plate_type" class="text-danger mt-2">{{
                    primary_errors.vehicle_plate_type[0] }}</span>
                </v-col>

                <v-col cols="6" dense>
                  <v-text-field :filled="!editable" label="Plate Color" dense small outlined clearable
                    autocomplete="off" v-model="payload_members.vehicle_plate_color" hide-details
                    :readonly="!editable"></v-text-field>
                  <span v-if="primary_errors && primary_errors.vehicle_plate_color" class="text-danger mt-2">{{
                    primary_errors.vehicle_plate_color[0] }}</span>
                </v-col>
                <v-col cols="6" dense>

                  <v-select label="Plate Size" :disabled="!editable" v-model="payload_members.plate_size" :items="[
                    {
                  ...
                  ]" dense placeholder="Number Plate Size" outlined :hide-details="true" item-text="description"
                    item-value="name"></v-select>
                </v-col>
                <v-col cols="6" dense>
                  <v-text-field :filled="!editable" label="Vehicle Type/Model" dense small outlined clearable
                    autocomplete="off" v-model="payload_members.vehicle_type" hide-details
                    :readonly="!editable"></v-text-field>
                  <span v-if="primary_errors && primary_errors.vehicle_type" class="text-danger mt-2">{{
                    primary_errors.vehicle_type[0] }}</span>
                </v-col>
                <v-col cols="6" dense>
                  <v-text-field :filled="!editable" label="Vehicle   Color" dense small outlined clearable
                    autocomplete="off" v-model="payload_members.vehicle_color" hide-details
                    :readonly="!editable"></v-text-field>
                  <span v-if="primary_errors && primary_errors.vehicle_color" class="text-danger mt-2">{{
                ...
                <v-col cols="6" dense>
                  <v-text-field :filled="!editable" label="Parking Slot" dense small outlined clearable
                    autocomplete="off" v-model="payload_members.parking_slot" hide-details
                    :readonly="!editable"></v-text-field>
                ...
                <v-col cols="6" dense>
                  <v-text-field :filled="!editable" label="Password" dense small outlined clearable autocomplete="off"
                    v-model="payload_members.password" hide-details type="password"
                    :readonly="!editable"></v-text-field>
                  <span v-if="primary_errors && primary_errors.password" class="text-danger mt-2">{{
                    primary_errors.password[0] }}</span>
                </v-col><v-col cols="6" dense>
                  <v-text-field :filled="!editable" label="Confirm Password" dense small outlined clearable
                    autocomplete="off" v-model="payload_members.confirm_password" hide-details type="password"
                    :readonly="!editable"></v-text-field>
                ...
                <v-col cols="6" dense>
                  ...
                  <v-radio-group :filled="!editable" class="radiogroup1" v-model="is_active">

                    <v-radio label="Parking Allowed" :value="true" />
                    <v-radio label="Blocked" :value="false" style="font-size: 10px" />
                  </v-radio-group>
                ...
                <v-col cols="6" dense v-if="!is_active">
                  ...
                  <v-text-field :filled="!editable" label="Reason for Blocking " dense small outlined clearable
                    autocomplete="off" v-model="payload_members.blocked_reason" hide-details
                    :readonly="!editable"></v-text-field>
...
export default {
  ...
  created() {
    ...
    if (this.editId != "" && this.item) {
      ...
      this.is_active = this.payload_members.is_active = this.item.is_active ? true : false;
      ...
      this.payload_members.vehicle_country_region = this.item.vehicle_country_region;
