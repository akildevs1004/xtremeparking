<template>
  <v-app>
    ...
    <v-list>
          ...
          <v-list-item style="height: 30px; padding-left: 5px; width: 300px" :class="notificationsMenuItems.length > 0 &&
            ...
            <v-list-item-content>
              <v-list-item-title class="black--text align-left text-left">
                <v-row>
                  <v-col cols="2" class="align-right text-right pr-1"><img :src="'/device-icons/' + item.icon"
                      style="width: 20px; vertical-align: middle" /></v-col>
                  <v-col cols="10">
      ...
      <v-menu nudge-bottom="50" transition="scale-transition" origin="center center" bottom left min-width="200"
        ...
        <v-list light nav dense>
          <v-list-item-group color="primary">
            <v-list-item v-if="$auth && $auth.user?.user_type == 'company'" @click="goToCompany()">
              <v-list-item-icon>
      ...
      <v-dialog v-model="dialogAlarmPopupNotificationStatus" transition="dialog-top-transition" max-width="900"
        style="z-index: 9999">
        <!-- <template v-slot:activator="{ on, attrs }">
        ...
        <template v-slot:default="dialog">
          <v-card style="z-index: 9999">
            ...
            <v-card-text style="padding-left: 0px">
              <AlarmPopupAllAlarmEvents v-if="notificationAlarmDevicesContentData"
                :items="notificationAlarmDevicesContentData"
                @callwait5MinutesNextNotification="wait5MinutesNotification" :key="popupKey" :alarm_icons="alarm_icons"
      ...
      try {
        ...
        this.notificationsMenuItems = data.map((element) => ({
          title: element.device?.customer?.building_name
            ? `${element.device.customer.building_name} - ${element.alarm_type ?? "Unknown"
            }`
            : "---",
