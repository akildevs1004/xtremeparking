          ...
          <div style="color: #fff; text-align: left" class="pt-8">
            <v-form :style="'padding: 20px;' + bgcolor" ref="form" method="post" v-model="valid" lazy-validation
              ...
              <div class="form-outline pt-5">
                ...
                <v-text-field hide-details role="presentation" dense light outlined clearable autocomplete="off"
                  ...
                  @click:append="show_password = !show_password" style="
                    background-color: #fff !important;
                    color: black !important;
                    margin-top: 5px;
                  "></v-text-field>
  ...
  methods: {
    ...
    async login() {
      ...
      if (this.$refs.form.validate()) {
        ...
        this.msg = "";
        this.loading = true;
        // Example login method in your component
        ...
        try {
          ...
          let data = await mqttRequestReply({
            ...
            payload: {
              ...
              source: 'admin', // keep parity with your API
            },
            timeoutMs: 1000 * 30,
          });
          ...
          if (data.action == 'login') {
            ...
            if (resp?.status !== 'success' || !resp.token || !resp.user) {
              const msg = resp?.message || 'Login failed. Please try again.';
              this.snackbar = true;
            ...
            if (this.$auth?.setUserToken) {
              await this.$auth.setUserToken(resp.token);
              await this.$auth.setUser(resp.user);
            } else {
              localStorage.setItem('token', resp.token);
              if (this.$axios?.setToken) this.$axios.setToken(resp.token, 'Bearer');
            }

            // Mobile-first route
            const isMobile = typeof window !== 'undefined' && window.innerWidth < 700;
            if (isMobile) {
              this.$router.replace('/alarm/mobiledashboard');
              return;
            }

            // Role routing (user_type from your /api/me sample is "member")
            const role = (resp.user.user_type || '').toLowerCase();
            const roleRoutes = {
            ...
            this.$router.replace(roleRoutes[role] || '/master');

          }
        } catch (e) {
          console.log('MQTT login error:', e);
          const msg = 'Network error: unable to reach the server.';
          this.snackbar = true;
          this.snackbarMessage = msg;
          this.msg = msg;
        } finally {
