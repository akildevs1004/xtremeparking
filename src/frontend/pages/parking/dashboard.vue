  ...
  <div v-else-if="!isMobileDevice">
    ...
    <v-dialog v-model="dialogBlocked" max-width="700px" persistent>
      <v-card>
        ...
        >
          Information - {{ $utils.caps(vehicleStatusEntryExit) }} - blocked
          vehicle
          <v-spacer></v-spacer>
        ...
        <v-card-text style="margin: auto">
          <div class="pa-2">
            <v-icon color="green" v-if="gatePassStatus"
    ...
    <v-dialog v-model="dialogParkingFull" width="500px">
      <v-card style="padding: 0px !important">
        <v-card-title
          dense
          class="popup_background"
          style="background-color: red !important; padding: 0px !important"
        >
          Parking is Full
          <v-spacer></v-spacer>
        ...
        <v-card-text>
          <v-img src="/parking-full.png" cover></v-img> </v-card-text
      ></v-card>
    </v-dialog>
    ...
    <v-dialog v-model="dialogImagePreview" max-width="600px">
      <v-card>
        ...
        <v-card-text>
          <v-img
            :src="dialogImageUrl"
            @click:outside="dialogImagePreview = false"
          ></v-img> </v-card-text
      ></v-card>
    </v-dialog>
    <v-row class="dashboard">
      <v-col style="max-width: 16.66%">
        <AudioSoundPlay
          v-if="activeAudio"
          :notificationsMenuItemsCount="mqttNewMessage ? 1 : 0"
        />
        <v-card
          elevation="2"
          style="height: 130px; border-radius: 0.5rem"
          class="blue-border-bottom custom-card"
          ><v-card-text
            ><v-row style="height: 130px">
              ...
              >
                <div
                  class="blue-text"
                  :key="statsKey"
                  :mqttNewMessage="mqttNewMessage"
                  style="font-size: 40px"
                >
                  {{ statisstics ? statisstics.total_parking_count : 0 }}
                </div>
                <br />
      ...
      <v-col style="max-width: 16.66%">
        <v-card
          elevation="2"
          style="height: 130px; border-radius: 0.5rem"
          class="red-border-bottom custom-card"
          ><v-card-text
            ><v-row style="height: 130px">
              <v-col
                class="d-flex1 justify-center"
              ...
              >
                <div
                  ...
                  style="font-size: 40px"
                >
                  {{ statisstics ? statisstics.total_parked : 0 }}
                </div>
                <br />
      ...
      <v-col style="max-width: 16.66%">
        <v-card
          elevation="2"
          style="height: 130px; border-radius: 0.5rem"
          class="green-border-bottom custom-card"
          ><v-card-text
            ><v-row style="height: 130px">
              <v-col
                class="d-flex1 justify-center"
              ...
              >
                <div
                  ...
                  style="font-size: 40px"
                >
                  {{ statisstics ? statisstics.total_available : 0 }}
                </div>
                <br />
      ...
      <v-col style="max-width: 16.66%">
        <v-card
          elevation="2"
          style="height: 130px; border-radius: 0.5rem"
          class="yellow-border-bottom custom-card"
          ><v-card-text
            ><v-row style="height: 130px">
              <v-col
                class="d-flex1 justify-center"
              ...
              >
                <div
                  ...
                  style="font-size: 40px"
                >
                  {{ statisstics ? statisstics.vehicle_count_today : 0 }}
                </div>
                <br />
      ...
      <v-col style="max-width: 16.66%">
        <v-card
          elevation="2"
          style="height: 130px; border-radius: 0.5rem"
          class="darkgreen-border-bottom custom-card"
          ><v-card-text
            ><v-row style="height: 130px">
              <v-col
                class="d-flex1 justify-center"
              ...
              >
                <div
                  ...
                  style="font-size: 40px"
                >
                  {{ statisstics ? statisstics.total_payments : 0 }}
                </div>
                <br />
          ...
          ><v-card-text>
            ...
            <v-row>
              <v-col
                ...
                "
              >
                <v-icon size="60" style="margin-top: -28px" color="yellow"
                  >mdi-boom-gate</v-icon
                >
              </v-col>
              <v-col
                style="
...
export default {
  ...
  mounted() {
    try {
      if (window)
        if (
          window.__APP_CONFIG__.PARKING_MODE == false ||
          window.__APP_CONFIG__.PARKING_MODE == "false"
        ) {
      ...
      response: {
        record: {
          ...
          in_background_file_name:
            "20250908200916528_F53310_VEHICLE_DETECTION_XTP100002_FORWARD_EMI_BACKGROUND.jpg",
          ...
          gate_open_automatically:
            "Tenant  is Active  - Gate Open Automatically",
  ...
  methods: {
    ...
    async initMqtt() {
      ...
      if (this.$env?.settings) {
        ...
        console.log(
          " this.$env.settings.MQTT_SOCKET_HOST)",
          this.$env.settings.MQTT_SOCKET_HOST
        );
        ...
        if (
          this.$env.settings.MQTT_SSL &&
          this.$env.settings.MQTT_SSL == true
        ) {
          options.protocol = "wss";
        }
        ...
        this.client.on("message", (topic, message) => {
          ...
          try {
            ...
            if (!this.mqttNewMessage.response.status) {
              //error
              this.snackbar = true;
              this.snackbarColor = "red";

              this.vehicle_notification_status =
                this.mqttNewMessage.response.message;
              this.response = this.mqttNewMessage.response.record.message;

              this.vehicleStatusEntryExit = "entry";

              let vehicleGustNoEntryImage =
                this.mqttNewMessage.response.record.image.replace(
                  "_BACKGROUND",
                  "_VEHICLE"
                );

              console.log(
                "this.mqttNewMessage.response",
                this.mqttNewMessage.response.record
              );

              let LiveImagePath =
                "http://" +
                this.$env.settings.BACKEND_URL2 +
                "/api/parking_camera_logs/" +
                this.$auth.user.company_id +
                "/";

              LiveImagePath = LiveImagePath.replace("_BACKGROUND", "_VEHICLE");
            ...
            if (this.mqttNewMessage.response.record.out_time == null) {
              ...
              if (this.mqttNewMessage.response.record.in_background_file_name) {
                this.mqttNewMessage.response.record.image_vehicle =
                  this.mqttNewMessage.response.record.public_image_url +
                  "/" +
                  this.mqttNewMessage.response.record.in_background_file_name.replace(
                    "_BACKGROUND",
                    "_VEHICLE"
                  );
              ...
              if (
                this.mqttNewMessage.response.record.out_background_file_name
              ) {
                this.mqttNewMessage.response.record.image_vehicle =
                  this.mqttNewMessage.response.record.public_image_url +
                  "/" +
                  this.mqttNewMessage.response.record.out_background_file_name.replace(
                    "_BACKGROUND",
                    "_VEHICLE"
                  );
            ...
            if (
              this.mqttNewMessage?.response.record.membership_status ==
              "Membership Expired"
            ) {
              this.snackbar = true;
              this.snackbarColor = "red";
            ...
            if (this.mqttNewMessage?.response.record.gate_open_automatically) {
              setTimeout(() => {
                ...
                this.snackbarColor = "green";
                this.response =
                  this.mqttNewMessage?.response.record.gate_open_automatically;
            ...
            } else this.gatePassStatus = false;
            setTimeout(() => {
            ...
            this.response = `Error: ${
              ex.message || "Error occurred while processing MQTT message."
            }`;
          }
      ...
      if (confirm("Are you sure want to Update Payment?")) {
        try {
          ...
          if (data.status) {
            ...
            this.snackbarColor = "green";
            this.response =
              "Payment is updated successfully And Gate is Open Now";
