  ...
  <div>
    ...
    <v-dialog v-model="dialogViewTicket" width="700px" style="z-index: 9999">
      <v-card>
        ...
        <v-card-text>
          <ViewTicket :key="key" :editItem="editItem" @close_dialog="close_dialog_reaction"
    ...
    <v-card :height="browserHeight - 80">
      <v-card-text>
        <v-row v-if="!filterWord">
          <v-col class="pl-4">
            <h3 v-if="status">Active Tickets</h3>
          </v-col>
          ...
          <v-col v-if="technician_id == null && customer_id" class="pt-5" style="max-width: 80px">
          ...
          <v-col>
            <v-data-table :headers="headers" :items="items" :server-items-length="totalRowsCount" :loading="loading"
              ...
              <template v-slot:item.customer="{ item }">
                <div :class="getIsReadStatus(item) ? '' : 'bold'" v-if="item.customer">
                  {{ item.customer.building_name }}
                  <div class="secondary-value">Customer</div>
                </div>
                <div :class="getIsReadStatus(item) ? '' : 'bold'" v-else-if="item.security">
                  {{ item.security.first_name }} {{ item.security.last_name }}

                  <div class="secondary-value">Operator</div>
                </div>
                <div :class="getIsReadStatus(item) ? '' : 'bold'" v-else-if="item.technician">
                  Technician
              ...
              <template v-slot:item.ticket_responses="{ item }">
                <div :class="getIsReadStatus(item) ? '' : 'bold'" @click="addReply(item, false)">
              ...
              <template v-slot:item.closed_datetime="{ item }">
                <div v-if="item.status == 0" :class="getIsReadStatus(item) ? '' : 'bold'">
                  {{
                    $dateFormat.formatDateMonthYear(item.last_active_datetime)
                  }}
                </div>
