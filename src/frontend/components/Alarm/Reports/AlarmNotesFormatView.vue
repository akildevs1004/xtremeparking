<template>
  <div>
        ...
        <v-row no-gutters>
          ...
          <v-col style="
            ...
            <div style="font-size: 13px; font-weight: bold">
              ...
              <span style="font-size: 10px">({{ alarm.device?.customer.buildingtype.name || "---" }})</span>
            </div>
            ...
            <div style="font-size: 12px">
              <v-icon size="15">mdi-at</v-icon>{{ alarm.device?.customer.user.email || "---" }},
            </div>
            <div style="font-size: 12px">
              <v-icon size="15">mdi-cellphone-basic</v-icon>{{ alarm.device?.customer.contact_number || "---" }}
            </div>
          </v-col>
        </v-row></v-col>
      <v-col style="border-left: 1px solid #ddd">
        <v-row no-gutters>
          ...
          <v-col style="
            ...
            <div style="font-size: 12px">
              <v-icon size="15">mdi-at</v-icon>{{ alarm.device?.company.user?.email || "---" }}
            </div>
            <div style="font-size: 12px">
              <v-icon size="15">mdi-cellphone-basic</v-icon>{{ alarm.device?.company.contact_number || "---" }}
            </div>
          </v-col>
        </v-row>
      </v-col>
      <v-col style="border-left: 1px solid #ddd">
        <v-row no-gutters>
          ...
          <v-col style="
            ...
            <div style="font-size: 12px">
              <v-icon size="15">mdi-at</v-icon>{{ alarm.security?.email || "---" }}
            </div>

            <div style="font-size: 12px">
              <v-icon size="15">mdi-cellphone-basic</v-icon>{{ alarm.security?.contact_number || "---" }}
            </div>
          </v-col>
        </v-row>
      </v-col>
      <v-col style="min-width: 400px; border-left: 1px solid #ddd">
        <v-row>
          <v-col style="
              ...
              padding-bottom: 0px;
            ">
            <img :title="alarm.alarm_type" style="width: 30px" :src="getAlarmColorObject(alarm).image + '?4=3'" />
            <div>
              {{ alarm.alarm_type ?? "---" }},{{
              ...
              <tbody>
                ...
                <tr style="border-bottom: 1px solid #ddd">
                  ...
                  <td style="padding: 0 10px">
                    <span v-if="alarm.forwarded == true && alarm.alarm_status == 1">Forwarded</span>
                    <span v-else-if="alarm.alarm_status == 1">Open</span>
                    <span v-else-if="alarm.alarm_status == 0">Closed</span>
                    <v-icon v-if="alarm.forwarded === true && alarm.alarm_status == 1" title="Forwarded" color="orange"
                      style="color: red">mdi-lock-open-variant-outline</v-icon>

                    <v-icon v-else-if="alarm.alarm_status == 1" title="Open" colo="red"
                      style="color: red">mdi-lock-open-outline</v-icon>

                    <v-icon v-else-if="alarm.alarm_status == 0" title="Closed" color="green"
                      style="color: red">mdi-lock-outline</v-icon>
                  </td>
                </tr>
                <tr style="border-bottom: 1px solid #ddd">
    ...
    <div style="text-align: center; padding: 7%" v-if="alarm.notes.length == 0 && alarm.alarm_status == 1">
      ...
      <v-col class="alarm-notes-timeline">
        <template>
          <v-timeline v-if="alarm.notes.length > 0 || alarm.alarm_end_datetime">
            <v-timeline-item fill-dot color="white" v-for="(note, index) in alarm.notes" :key="'alarmnotes' + index"
              right>
              <template v-slot:opposite style="">
                <div style="padding-right: 10px">
                  {{ $dateFormat.formatDateMonthYear(note.created_datetime) }}
                </div>
                <div v-if="note.contact" style="
                    float: right;
                    width: 140px;
                    margin-right: -25px;
                    font-size: 10px;
                  ...
                  <div style="text-align: left">
                    ...
                    <div>
                      <v-icon size="15">mdi-cellphone-basic</v-icon>{{ note.contact.phone1 }}
                    </div>
                    <div>
                      <v-icon size="15">mdi-cellphone-basic</v-icon>{{ note.contact.phone2 }}
                    </div>
                    <div>
                      <v-icon size="15">mdi-at</v-icon>{{ note.contact.email }}
                    </div>
                    <div>
                      <v-icon size="15">mdi-whatsapp</v-icon>{{ note.contact.whatsapp }}
                    </div>
                  </div>
                </div>
              </template>
              <template v-slot:icon>
                <v-avatar>
                  <v-icon v-if="note.event_status == 'Forwarded'" title="Forwarded" color="orange"
                    style="color: red">mdi-lock-open-variant-outline</v-icon>
                  <v-img v-else-if="note.contact?.profile_picture" style="
                      border-radius: 50%;
                      height: 40px;
                      min-height: 40px;
                      width: 40px;
                      max-width: 40px;
                    " :src="note.contact?.profile_picture
              ...
              <v-card class="elevation-0" style="border: 1px solid #ddd">
                ...
                <v-card-text><span class="bold">Notes: </span>{{ note.notes }}
                  <v-divider v-if="note.event_status != 'Forwarded'"></v-divider>

                  <v-row v-if="note.event_status != 'Forwarded'">
                    <v-col class="bold">Call Status</v-col>
                    <v-col>: {{ note.call_status || "---" }}</v-col><v-col class="bold">Call Response</v-col>
                    <v-col>: {{ note.response || "---" }}</v-col><v-col class="bold">Event Status </v-col>
                    <v-col>: {{ note.event_status || "---" }}</v-col>
                  </v-row>
                </v-card-text>
              </v-card>
            </v-timeline-item>
            <v-timeline-item class="close-alarm" v-if="alarm.alarm_status == 0" fill-dot color="white" right>
