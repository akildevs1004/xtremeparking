      ...
      <v-card>
        ...
        <v-card-text>
          <v-container style="min-height: 100px">
            <AlarmForwardEvent name="AlramCloseNotes" :key="key" :customer_id="customer_id" @closeDialog="closeDialog"
              :alarm_id="eventId" :popupEventText="popupEventText" />
          </v-container>
        </v-card-text>
      </v-card>
    </v-dialog>
        ...
        <v-row class="mt-0" v-if="!eventFilter">
          ...
          <v-col class="text-right mt-1">
            <v-row>
              ...
              <v-col>
                <v-text-field style="padding-top: 7px; float: right" height="20" class="employee-schedule-search-box"
                  @input="getDataFromApi(0)" v-model="commonSearch" label="Common Search(All Content)"
            ...
            <v-tabs-items v-model="tab">
              <v-tab-item v-for="(item, index) in sensorItems" :key="item.id">
                <v-card color="basil" flat>
                  <v-card-text style="padding: 0px">
                    <v-data-table :name="'table' + index" v-if="showTable" :headers="headers" :items="items"
